...
# requirement following PEN testing. Finish with clearing the cache created during this to keep the image as small as
# possible (using Docker we can't benefit from using the cache)
RUN apk update \
  && apk upgrade \
  && rm -rf /var/cache/apk/*
...
# Set port to use. Default to port 3000 for node. Our docker-compose.yml overrides this to 3003 to avoid clashes with
# other environments
ARG PORT=3000
ENV PORT $PORT
...
# Set port to use. Default to port 3000 for node.
ARG PORT=3000
ENV PORT $PORT
EXPOSE $PORT
